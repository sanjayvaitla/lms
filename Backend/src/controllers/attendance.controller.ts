...
async function getTrainerPerms(uid: string) {
  const { rows } = await db.query(
    'SELECT * FROM trainer_permissions WHERE trainer_id = $1',
    [uid],
  );
  if (!rows.length) {
    return { canEditAttendance: false, canDeleteAttendance: false, canSoftDeleteOnly: false };
  }
  return {
    canEditAttendance:   rows[0].can_edit_attendance   as boolean,
    canDeleteAttendance: rows[0].can_delete_attendance as boolean,
...
export async function createSession(
  ...
  try {
    if (isTrainer(req)) {
      const perms = await getTrainerPerms(userId(req));
      if (!perms.canEditAttendance) {
        throw new AppError('You do not have permission to create sessions', 403, 'FORBIDDEN');
      }
    }
    const body = createSessionSchema.parse(req.body);
    const scopeTrainerUserId = isTrainer(req) ? userId(req) : undefined;
    const session = await svc.createSession(body, userId(req), scopeTrainerUserId);
    ok(res, session, 201);
  } catch (e) { next(e); }
}
...
export async function listSessions(
  ...
  try {
    ...
    const scopeTrainerUserId = isTrainer(req) ? userId(req) : undefined;
    const result = await svc.listSessions(query, scopeTrainerUserId);
    ok(res, result);
  } catch (e) { next(e); }
}
...
export async function updateSession(
  ...
  try {
    if (isTrainer(req)) {
      const perms = await getTrainerPerms(userId(req));
      if (!perms.canEditAttendance) {
        throw new AppError('You do not have permission to edit sessions', 403, 'FORBIDDEN');
      }
    }
...
export async function deleteSession(
  ...
  try {
    if (isTrainer(req)) {
      const perms = await getTrainerPerms(userId(req));
      if (!perms.canDeleteAttendance) {
        throw new AppError('You do not have permission to delete sessions', 403, 'FORBIDDEN');
      }
      // Soft-delete-only: mark session as CANCELLED instead of hard delete
      if (perms.canSoftDeleteOnly) {
        await db.query(
          `UPDATE attendance_sessions SET status = 'CANCELLED' WHERE id = $1`,
          [req.params['sessionId']],
        );
        return res.json({
          success: true,
          action: 'archived',
          message: 'Session cancelled (soft delete). Contact Super Admin for permanent deletion.',
        });
      }
    }
