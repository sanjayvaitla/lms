...
export function googleCallback(req: Request, res: Response, next: NextFunction) {
  ...
  passport.authenticate('google', { session: false }, async (err: Error | null, user: { id: string; role: string } | false) => {
    ...
    try {
      ...
      const clientUrl = process.env.CLIENT_URL ?? 'http://localhost:5173';
      // Use hash fragment so tokens are not sent to servers/proxies in Referer or access logs
      res.redirect(
        `${clientUrl}/auth/callback#accessToken=${encodeURIComponent(accessToken)}&refreshToken=${encodeURIComponent(refreshToken)}`,
      );
    } catch (e: unknown) {
      const clientUrl = process.env.CLIENT_URL ?? 'http://localhost:5173';
      if (e instanceof AppError && e.code === 'ACCOUNT_PENDING') {
        return res.redirect(`${clientUrl}/login?error=pending_approval`);
      }
      if (e instanceof AppError && (e.code === 'ACCOUNT_BLOCKED' || e.code === 'ACCOUNT_REJECTED')) {
        return res.redirect(`${clientUrl}/login?error=account_inactive`);
      }
      next(e);
    }
  })(req, res, next);
