  ...
  if (pgErr.code) {
    switch (pgErr.code) {
      ...
      case '22P02':
        ...
        return;
      case '23502': {
        const col = pgErr.column ?? pgErr.detail ?? '';
        const tbl = pgErr.table ?? '';
        console.error(`[23502 NOT_NULL] table=${tbl} column=${col}`, pgErr.message);
        res.status(400).json({
          success: false,
          message: `A required field is missing${col ? ` (${col})` : ''}.`,
          code: 'NOT_NULL',
        });
        return;
      }
