...
export class AssignmentSubmission extends Model {
  ...
  @AllowNull(true)
  @Column({ field: 'zip_key', type: DataType.TEXT })
  declare zipKey: string | null;

  @AllowNull(true)
  @Column({ field: 'github_fork_url', type: DataType.TEXT })
  declare githubForkUrl: string | null;

  @AllowNull(true)
  @Column({ field: 'github_latest_commit_sha', type: DataType.TEXT })
  declare githubLatestCommitSha: string | null;
  ...
  @Default('SUBMITTED')
  @Column(DataType.TEXT)
  declare status: 'IN_PROGRESS' | 'SUBMITTED' | 'GRADED' | 'LATE';
