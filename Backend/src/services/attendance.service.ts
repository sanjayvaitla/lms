...
import { trainerBatchAccessSql, assertTrainerBatchAccess } from '../lib/batch-access';

// ─────────────────────────────────────────────────────────────────────────────
// SESSIONS
// ─────────────────────────────────────────────────────────────────────────────

export async function createSession(input: CreateSessionInput, createdBy: string, trainerScopeId?: string) {
  // Verify batch exists
  const { rows: batchRows } = await db.query(
    'SELECT id, name, program_id, trainer_id FROM batches WHERE id = $1',
    [input.batchId],
  );
  if (!batchRows.length) throw new AppError('Batch not found', 404, 'NOT_FOUND');
  if (trainerScopeId) await assertTrainerBatchAccess(input.batchId, trainerScopeId);
  ...
  const { rows } = await db.query(
    ...
    [
      input.batchId,
      input.trainerId ?? batchRows[0].trainer_id ?? null,
      input.title,
      input.sessionDate,
...
export async function listSessions(query: ListSessionsQuery, scopeTrainerUserId?: string) {
  const conds: string[] = [];
  ...
  if (scopeTrainerUserId) {
    conds.push(trainerBatchAccessSql('b', `$${i++}`));
    params.push(scopeTrainerUserId);
  }
  ...
  const { rows } = await db.query(
    `SELECT
     ...
     JOIN programs c  ON c.id = b.program_id
     LEFT JOIN users u ON u.id = s.trainer_id
     LEFT JOIN attendance_records r ON r.session_id = s.id
     ${where}
     GROUP BY s.id, b.name, c.name, u.name
...
export async function getSession(id: string) {
  ...
  const { rows: records } = await db.query(
    `SELECT
     ...
     LEFT JOIN attendance_records r ON r.student_id = u.id AND r.session_id = $1
     LEFT JOIN users m ON m.id = r.marked_by
     WHERE e.batch_id = $2
     ORDER BY u.name ASC`,
    [id, sessionRows[0].batch_id],
  );
...
export async function markBulk(sessionId: string, input: MarkBulkInput, markedBy: string) {
  const { rows: session } = await db.query(
    'SELECT id, batch_id, trainer_id FROM attendance_sessions WHERE id = $1',
    [sessionId],
  );
  if (!session.length) throw new AppError('Session not found', 404, 'NOT_FOUND');
  const hasTrainer = !!session[0].trainer_id;

  await db.transaction(async (client) => {
    if (input.records.length > 0) {
      const studentIds = input.records.map(r => r.studentId);
      const statuses = input.records.map(r => r.status);
      const remarks = input.records.map(r => r.remarks ?? null);

      await db.query(
        `INSERT INTO attendance_records (session_id, student_id, status, marked_by, marked_at, remarks)
         SELECT $1, u.student_id, u.status, $5, NOW(), u.remarks
         FROM UNNEST($2::uuid[], $3::text[], $4::text[]) AS u(student_id, status, remarks)
        ...
        [sessionId, studentIds, statuses, remarks, markedBy],
        client,
      );
    }
...
export async function getDashboardStats() {
  ...
  // Last 30 calendar days — fill missing days with zeros so the trend spans a full month
...
export async function getBatchAttendance(batchId: string) {
  const { rows: batchRows } = await db.query(
    `SELECT b.id, b.name, c.name AS course_title
     FROM batches b JOIN programs c ON c.id = b.program_id
     WHERE b.id = $1`,
    [batchId],
  );
  if (!batchRows.length) throw new AppError('Batch not found', 404, 'NOT_FOUND');
...
export async function getLearnerAttendance(studentId: string) {
  ...
  const { rows: summary } = await db.query(
    `SELECT
       b.id AS batch_id, b.name AS batch_name, c.name AS course_title,
       COUNT(r.id)::int                                              AS total,
       COUNT(r.id) FILTER (WHERE r.status = 'PRESENT')::int         AS present,
       COUNT(r.id) FILTER (WHERE r.status = 'ABSENT')::int          AS absent,
       COUNT(r.id) FILTER (WHERE r.status = 'LATE')::int            AS late,
...
export async function getTrainerSessions(trainerId: string) {
  ...
  const { rows: stats } = await db.query(
    `SELECT
       COUNT(DISTINCT s.id)::int                                     AS total_sessions,
       COUNT(DISTINCT s.id) FILTER (WHERE s.status = 'COMPLETED')::int AS sessions_present,
  ...
  return {
    ...
    sessions: sessions.map((s: any) => ({
      ...formatSessionWithStats(s),
      trainerPresent: s.status === 'COMPLETED',
      trainerMarkedAt: null,
    })),
    stats: {
      totalSessions: Number(statsRow.total_sessions ?? 0),
      sessionsPresent: Number(statsRow.sessions_present ?? 0),
...
function formatSession(r: any) {
  return {
    ...
    notes: r.notes ?? null,
    status: r.status,
    trainerPresent: r.status === 'COMPLETED',
    trainerMarkedAt: null,
