 ...
 *   GOOGLE_CALLBACK_URL  (default: http://localhost:6000/api/v1/auth/google/callback)
  ...
  passport.use(new GoogleStrategy(
    {
      ...
      callbackURL: process.env.GOOGLE_CALLBACK_URL ?? 'http://localhost:6000/api/v1/auth/google/callback',
      scope: ['profile', 'email'],
    },
    async (
      _accessToken: string,
...
export async function upsertGoogleUser(profile: GoogleProfile) {
  ...
  if (existing.rows.length) {
    ...
    const { rows } = await db.query(
      `INSERT INTO users (name, email, google_id, auth_provider, avatar_url, role, password_hash, account_status)
       VALUES ($1, $2, $3, 'GOOGLE', $4, 'STUDENT', NULL, 'PENDING')
...
export async function issueTokensForUser(userId: string, role: string) {
  const { assertUserCanAuthenticate, issueTokenPair } = await import('../lib/tokens');
  // Re-check status so PENDING Google signups never get sessions
  const user = await assertUserCanAuthenticate(userId);
  return issueTokenPair(user.id, user.role || role);
}
