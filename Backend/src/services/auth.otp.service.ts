...
export async function verifyOtp(
  ...
  if (existingUser.rows.length) {
    ...
    const { rows: newRows } = await db.query(
      `INSERT INTO users (name, email, phone_number, is_phone_verified, auth_provider, role, password_hash, account_status)
       VALUES ($1, $2, $3, TRUE, 'PHONE', 'STUDENT', NULL, 'PENDING')
  ...
  // PENDING / blocked learners must not receive sessions
  const { assertUserCanAuthenticate } = await import('../lib/tokens');
  try {
    await assertUserCanAuthenticate(userId);
  } catch (err) {
    if (err instanceof AppError && err.code === 'ACCOUNT_PENDING') {
      throw new AppError(
        'Phone verified. Your account is pending fee admin approval — you can sign in after activation.',
        403,
        'ACCOUNT_PENDING',
      );
    }
  ...
  const { accessToken, refreshToken } = await issueTokenPair(userId, role);
