...
import { Search, Bell, Settings, Sparkles, Menu } from 'lucide-react';
...
export function Header({ onSearch, onMenuToggle }: HeaderProps) {
  ...
  return (
    <header className="h-14 bg-white border-b border-gray-100 px-4 sm:px-6 flex items-center gap-3 sm:gap-4 shrink-0 shadow-sm">
      {/* Mobile Menu Toggle */}
      <button 
        onClick={onMenuToggle}
        className="p-2 -ml-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg lg:hidden transition-colors"
      >
      ...
      {/* AI Tools button */}
      <button className="hidden sm:flex items-center gap-2 px-3 py-2 text-sm font-medium text-purple-600 bg-purple-50 border border-purple-200 rounded-lg hover:bg-purple-100 transition-colors">
