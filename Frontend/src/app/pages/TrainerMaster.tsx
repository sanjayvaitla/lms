...
function normalizePhoneToDigits(phone?: string): string | undefined {
...
// ── Trainer Card ──────────────────────────────────────────────────────────────
function TrainerCard({ trainer, canEdit, onEdit, onDelete, onProfile }: {
  trainer: Trainer;
  canEdit: boolean;
  onEdit: () => void;
  onDelete: () => void;
  ...
  async function onSubmit(values: CreateTrainerForm) {
    try {
      await api.post('/trainers', {
        ...values,
        phone: normalizePhoneToDigits(values.phone),
      });
...
function EditTrainerModal({ trainer, onClose, onSuccess }: {
  ...
  const {
    ...
    defaultValues: {
      ...
      phone:    normalizePhoneToDigits(trainer.phone ?? undefined) ?? '',
    },
  });
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              {errors.name && <p className={ERROR_CLS}>{errors.name.message as string}</p>}
            </div>
            <div>
...
function TrainerProfileDrawer({ trainerId, onClose, onEdit }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div
        className={`relative bg-white h-full shadow-2xl flex flex-col overflow-y-auto transition-all duration-300 ${
          maximized ? 'w-full' : 'w-full max-w-md'
        }`}
      >
        ...
        <div className="p-6 border-b bg-gradient-to-r from-teal-50 to-emerald-50">
          <div className="flex justify-end gap-1 mb-3">
            <button onClick={() => setMaximized((v) => !v)} className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-white/60 rounded-lg" title={maximized ? 'Minimize' : 'Maximize'}>
            ...
            <div>
              ...
              {trainer.phone && (
                <p className="text-xs text-gray-400 flex items-center gap-1 mt-0.5">
                  <Phone className="w-3 h-3" /> +91 {trainer.phone.replace(/^\+?91/, '')}
                </p>
              )}
            </div>
          </div>
