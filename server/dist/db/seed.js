...
function monthsAgo(n, plusDays = 0) {
    const d = new Date();
...
async function main() {
    ...
    const hash = await bcryptjs_1.default.hash(PWD, SALT_ROUNDS);
    // ── Super Admin ──────────────────────────────────────────────────────────────
    await q(`INSERT INTO users (name, email, password_hash, role) VALUES ($1,$2,$3,'SUPER_ADMIN')`, ['Ravi Shankar', 'ravi@vtricks.com', hash]);
    ...
    for (const t of trainerData) {
        const res = await q(`INSERT INTO users (name, email, password_hash, role) VALUES ($1,$2,$3,'TRAINER') RETURNING id`, [t.name, t.email, hash]);
        const tid = res.rows[0].id;
        trainerIds.push(tid);
        await q(`INSERT INTO trainer_profiles (user_id, bio, skills) VALUES ($1,$2,$3)
       ON CONFLICT (user_id) DO UPDATE SET bio=$2, skills=$3`, [tid, t.bio, t.skills]);
    }
    ...
    for (const c of courseData) {
        const res = await q(`INSERT INTO courses (title, category, status, level, duration_months, description, trainer_id, color_token)
       VALUES ($1,$2,'ACTIVE',$3,$4,$5,$6,$7) RETURNING id`, [c.title, c.category, c.level, c.months, `Learn ${c.title} from scratch.`, trainerIds[c.trainerIdx], c.color]);
    ...
    for (let i = 0; i < studentNames.length; i++) {
        ...
        const res = await q(`INSERT INTO users (name, email, password_hash, role) VALUES ($1,$2,$3,'STUDENT') RETURNING id`, [studentNames[i], `${emailName}@vtricks.com`, hash]);
    ...
    for (const b of batchData) {
        const res = await q(`INSERT INTO batches (name, course_id, start_date, end_date, capacity, status)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`, [b.name, courseIds[b.courseIdx], b.start, b.end, 30, b.status]);
    ...
    for (const e of enrollmentPlan) {
       ...
       VALUES ($1,$2,$3,$4)
       ON CONFLICT (batch_id, student_id) DO NOTHING`, [batchIds[e.batchIdx], studentIds[e.studentIdx], e.completion, enrolledAt]);
    }
    ...
    for (let i = 0; i < moduleTitles.length; i++) {
        ...
        const res = await q(`INSERT INTO course_modules (course_id, title, description, sort_order, status)
       VALUES ($1,$2,$3,$4,$5) RETURNING id`, [mernCourseId, moduleTitles[i], `Module ${i + 1} content`, i, status]);
        moduleIds.push(res.rows[0].id);
    }
    // Complete module 0 so its quiz is released
    await q(`UPDATE course_modules SET status = 'COMPLETED', completed_at = NOW(), completed_by = $2 WHERE id = $1`, [moduleIds[0], rajeshId]);
    await q(`UPDATE course_modules SET status = 'RELEASED' WHERE id = $1`, [moduleIds[1]]);
    const questions = [
    ...
    for (const qu of questions) {
        await q(`INSERT INTO quiz_questions (course_id, module_id, question_text, question_type, options, correct_answer, points)
    ...
    for (let i = 0; i < moduleIds.length; i++) {
        ...
        await q(`INSERT INTO quizzes (course_id, module_id, title, questions_per_attempt, passing_score, randomize_questions, randomize_options, status, created_by)
       VALUES ($1,$2,$3,3,60,true,true,$4,$5)`, [mernCourseId, moduleIds[i], `Quiz: ${moduleTitles[i]}`, quizStatus, rajeshId]);
    }
    ...
    for (let i = 0; i < assignmentsData.length; i++) {
        ...
        const res = await q(`INSERT INTO assignments (course_id, module_id, title, max_score, status, due_date, pdf_filename, pdf_path, pdf_size_bytes)
       VALUES ($1,$2,$3,$4,'PUBLISHED',$5, 'demo.pdf', 'assignments/demo.pdf', 1024) RETURNING id`, [mernCourseId, moduleIds[i], a.title, a.maxScore, a.dueDate]);
        ...
        for (const bId of courseBatches) {
            await q(`INSERT INTO assignment_batches (assignment_id, batch_id) VALUES ($1,$2)
         ON CONFLICT DO NOTHING`, [assignId, bId]);
        }
    }
